...
MAX_TWEETS = 500000
RATE_LIMIT = MAX_TWEETS / (2 * 24 * 60)
print(0.8 * MAX_TWEETS)
class CustomStreamingClient(StreamingClient):
    ...
    def __init__(self, write=print, **kwds):
        ...
        self.tweets_since_last_timestamp = 0	
    ...
    def on_data(self, raw_data):
        ...
        time_elapsed = (datetime.datetime.now() - self.last_timestamp).seconds
        ...
        if self.tweets_since_last_timestamp >= RATE_LIMIT:	
            time.sleep(60 - time_elapsed)

        self.write(raw_data)
        self.total_tweets += 1
        self.tweets_since_last_timestamp += 1
    ...
    try:
        if flags.filter:
            ...
            while True and not(twitter_streaming_client.is_maxed_out() or twitter_streaming_client.is_sunset()):
                time.sleep(1)
                twitter_streaming_client.sample(tweet_fields=['created_at'], expansions=['author_id', 'referenced_tweets.id.author_id'])
